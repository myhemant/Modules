...
(function ($, Drupal) {
  Drupal.behaviors.PgwsliderView = {
    attach: function (context, settings) {
        var pgwsettingsView = settings.PgwsliderView;
     $(context).find('body').once('once').each(function () {
        for (var i in pgwsettingsView) {
            var  $pgw_autoplay        = pgwsettingsView[i].$pgw_autoplay;
            var  $pgw_arrownavigator  = pgwsettingsView[i].$pgw_arrownavigator;
            var  $pgw_displaylist     = pgwsettingsView[i].$pgw_displaylist;
            var  $pgw_caption         = pgwsettingsView[i].$pgw_caption;
            var  $autoplay_interval   = pgwsettingsView[i].$pgw_autoplayinterval;
            var  $pgwdisplay_count    = pgwsettingsView[i].$pgwdisplay_count;
            var  $pgwtransition_effect= pgwsettingsView[i].$pgwtransition_effect;
         }
...
;(function($){
    $.fn.pgwSlideshow = function(options) {
        var defaults = {
            ...
            transitionEffect : $pgwtransition_effect,
            displayList : $pgw_displaylist,
            displayControls : $pgw_arrownavigator,
            touchControls : true,
            autoSlide : $pgw_autoplay,
            beforeSlide : false,
            afterSlide : false,
            maxHeight : null,
            adaptiveDuration : 200,
            transitionDuration : 500,
            intervalDuration : $autoplay_interval,
            displayCaption : $pgw_caption
        };
        ...
        var setup = function() {
            ...
            pgwSlideshow.plugin = pgwSlideshow.plugin.parent();
            var captionshow  = ''; 
            if(pgwSlideshow.config.displayCaption){ captionshow = '<span class="ps-caption"></span>'; } 
            pgwSlideshow.plugin.prepend('<div class="ps-current"><ul></ul>'+captionshow+'</div>');
            ...
            if (pgwSlideshow.slideCount > 1) {
				...
				// Setup the slider bar patch
				pgwSlideshow.plugin.find('.ps-list').prepend('<div class="slidebar-container"><div class="slidebar">&nbsp;</div></div>');
        ...
        var finishElement = function(element) {
			// Element caption
            var elementText = '';
            // show the slide count number on the caption
            if($pgwdisplay_count){    
		elementText += '<center>'+element.id +' of '+pgwSlideshow.slideCount + '</center>';
            }    
            
            var slidesSlices = 100/pgwSlideshow.slideCount;
            var widthIncr = slidesSlices*element.id;
            ...
            if (elementText != '') {
                ...
                if (typeof pgwSlideshow.plugin.find('.ps-caption').fadeIn == 'function') {  
					...
					pgwSlideshow.plugin.find('.slidebar').css("background-color",'blue')
					pgwSlideshow.plugin.find('.slidebar').css("width", widthIncr+'%')
			...
			//pgwSlideshow.plugin.find('.ps-list ul').hide();
            return true;
        }
